...
def word2features(sent, i, ft):
    ...
    # adding word embedding features (one for each vector component)
    for iv, value in enumerate(embedding):
        features['v{}'.format(iv)] = value

    return features

def sent2features(sent, ft):
    return [word2features(sent, i, ft) for i in range(len(sent))]
...
def get_max_len(dataset):
    return max(len(x) for x in dataset)

def process_input_data(data_dir):

    df = pd.read_csv(data_dir)

    # EXTRA PREPROCESSING
    df = extra_preprocessing(df)

    getter_set = SentenceGetter(df)
    sentences = [[word[0] for word in sentence] for sentence in getter_set.sentences]
    labels = [[s[2] for s in sentence] for sentence in getter_set.sentences]
    tag_set = set(df["tag"].values)
