...
def AnnotationSet_to_dataset_format(AnnotationSet, output_dir, BO_tags = False, labels_to_suppress = []):
    ...
    for i in tqdm(range(1, len(iaa_dict))): 
...
def get_worker_accuracies_dict(worker_accuracies_dir):
    """ returns a dict of worker accuracies from directory """

    worker_accuracies_df = pd.read_csv(worker_accuracies_dir)

    # drop the 'positive labels' column - not needed
    worker_accuracies_df = worker_accuracies_df.drop('positive labels', axis=1)

    return worker_accuracies_df.set_index('worker_id').T.to_dict('records')[0]

def get_ranked_worker_indices(worker_list_dir, worker_accuracies_dir):
    """ returns a list of indices in decreasing priority,
    taken from ranking worker accuracies from qualification task """

    # first, get worker list
    with open(worker_list_dir) as f:
        content = f.readlines()
    worker_list = [x.strip() for x in content]

    # second, get worker accuracies
    worker_accuracies_dict = get_worker_accuracies_dict(worker_accuracies_dir)

    subset_worker_accuracies_dict = {key: value for key, value in worker_accuracies_dict.items() if key in worker_list}

    sorted_subset_workers = sorted(subset_worker_accuracies_dict, key = lambda key: subset_worker_accuracies_dict[key])

    ranked_indices = []
    for i in range(len(sorted_subset_workers)):
        worker_id = sorted_subset_workers[i]
        worker_index = worker_list.index(worker_id)
        ranked_indices.append(worker_index)

    return ranked_indices

def txt_to_df(file_dir, method, priority_list=[], worker_indices=[], pos_tags = True):
    ...
    if pos_tags:
        with open(file_dir) as f:
            ...
            for line in tqdm(f, total=num_lines):
                if line.split(): # i.e. not line break
                    ...
                    # for token in nlp(" ".join(rolling_sentence_list)):
        ...
        # print(f'sentence_number_list: {len(sentence_number_list)}, {sentence_number_list[:5]}')
        # print(f'word_list: {len(word_list)}, {word_list[:5]}')
        # print(f'pos_list: {len(pos_list)}, {pos_list[:5]}')
        # print(f'tag_list: {len(tag_list)}, {tag_list[:5]}')
        ...
        return pd.DataFrame(
            {
                'sentence_idx': sentence_number_list,
                'word': word_list,
                'tag': tag_list
            }
        )
